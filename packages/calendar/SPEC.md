...
interface CalendarOptions {
  defaultView?: 'day' | 'week' | 'month' | 'multiDay' | 'agenda' | 'scheduler' | 'timeline';
...
- **空白网格创建（触控长按）**：时间网格 / 月格 / timeline 资源行的「拖拽创建」在触控下需**长按约 400ms** 才进入创建。长按激活时同步触发触觉反馈（`triggerHapticFeedback`）并立即显示单格临时卡片（选区高亮）作为视觉确认；若用户随后拖拽则选区自然扩展；若直接松手则卡片保持可见，**松手后 200ms** 打开创建弹窗、选区消失。**桌面单击空白格**同样触发单格创建（对标 Google Calendar 行为）。长按前的滑动仍是原生滚动，此设计让「滚动看时段」与「长按/拖拽创建」在同一表面共存，对标 iOS 日历 / Mobiscroll mobile day view。鼠标 / 手写笔不经长按，行为与桌面一致（零回归）。
