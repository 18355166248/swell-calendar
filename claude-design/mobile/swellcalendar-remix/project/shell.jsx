...
function Sidebar({ view, setView, openCreate, sidebar }) {
  const navMain = [
  ...
  return (
    <aside className="sidebar">
      <div className="brand">
        <div className="brand-mark"><Ic.swell /></div>
      ...
      <button className="side-cta" onClick={openCreate}>
        <Ic.plus /><span className="lbl">新建日程</span>
      </button>
      <nav className="nav">
        ...
        {navCal.map((n) => (
          <button key={n.c} className="nav-item">
            <span style={{ width: 18, display: "grid", placeItems: "center" }}>
              <span style={{ width: 11, height: 11, borderRadius: 4, background: CAT_COLORS[n.c] }} />
            </span>
      ...
      <div className="side-foot">
        <div className="user-chip">
          <div className="avatar" style={{ background: "var(--cat-magenta-line)" }}>陈</div>
...
function MiniCalendar() {
  // March 2025-ish; week of 18-24 highlighted, 21 = today
  const dow = ["一", "二", "三", "四", "五", "六", "日"];
  const firstOffset = 5; // month starts on Saturday
  const cells = [];
  for (let i = 0; i < 42; i++) {
    const dnum = i - firstOffset + 1;
    const inMonth = dnum >= 1 && dnum <= 31;
    const inWeek = dnum >= 18 && dnum <= 24;
    const today = dnum === 21;
    cells.push({ dnum, inMonth, inWeek, today });
  }
  return (
    <div className="mini">
      ...
      <div className="mini-grid">
        {dow.map((d) => <div key={d} className="mini-dow">{d}</div>)}
        {cells.map((c, i) => (
          <div key={i} className={"mini-day" + (c.inMonth ? "" : " dim") + (c.today ? " today" : c.inWeek ? " in-week" : "")}>
...
function Topbar({ view, setView, toolbar, toggleRail, title, sub }) {
  const views = [
  ...
  return (
    <header className="topbar">
      <button className="tb-rail-toggle" onClick={toggleRail} title="切换侧栏"><Ic.sidebar /></button>
      ...
      <div className="tb-nav">
        ...
        <button className="tb-arrow"><Ic.chevL /></button>
        <button className="tb-arrow"><Ic.chevR /></button>
      </div>
      <div className="tb-spacer" />
      {toolbar !== "minimal" && (
        <div className="tb-search">
          <Ic.search /><input placeholder="搜索日程、与会人…" />
        </div>
      )}
      <div className={segClass}>
        {views.map((v) => (
          <button key={v.id} className={"seg-btn" + (view === v.id ? " active" : "")} onClick={() => setView(v.id)}>
            <v.icon />{toolbar !== "minimal" && <span>{v.label}</span>}
          </button>
        ))}
      </div>
      <button className="tb-icon-btn"><span className="dot" /><Ic.bell /></button>
      <button className="tb-icon-btn"><Ic.settings /></button>
    </header>
  );
}
