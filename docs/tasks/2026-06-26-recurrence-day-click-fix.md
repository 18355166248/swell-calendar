...
- 月视图事件过多时桌面端 `+N 更多` 点击被月格父层 pointerdown 抢走，浮层不稳定；移动端自绘月视图未渲染 `+N 更多` 入口。
- 桌面端和移动端部分入口不支持删除：删除函数只依赖详情弹层 `pick`，移动端直接进入编辑页、更多列表行等路径没有可用目标事件。
...
- 桌面月视图 `+N 更多` 在按钮层拦截 pointerdown，避免触发父层新建/拖拽；移动端月视图按天渲染 `+N 更多` chip，并复用已有更多事件 sheet 展示当天列表。
- 删除逻辑改为可接收目标 EventObject，详情弹层、编辑页、更多列表行共用同一删除入口；重复实例删除继续弹作用域选择，普通事件直接删除。
...
- in-app browser：桌面月视图 1012×1023 viewport 下点击 `+10 更多`，出现 `.more-events-pop`，标题为 `6月22日`
- in-app browser：移动端 390×844 viewport 下月视图出现 `.m-month-more-chip`，点击 `+10 更多` 后出现 `.more-events-pop--sheet`
- in-app browser：桌面月视图 `+10 更多` 浮层内每行出现「删除 / 编辑」操作按钮
- `./node_modules/.bin/tsc --noEmit -p apps/swell-calendar-s2/tsconfig.json`：通过，覆盖编辑页 / 更多列表删除回调类型检查
- 桌面端更多日程弹层改为 `min(520px, 100vw - 32px)` 响应式宽度，并将标题、时间、操作按钮拆成稳定列，避免删除/编辑按钮挤压标题；移动端 sheet 仍保持全宽。
- in-app browser：桌面月视图 1211×1023 viewport 下 `+N 更多` 弹层实测宽度约 514px，非 sheet，前 3 行标题未被按钮挤压；移动端 390px 级别 viewport 下仍进入 `.app--mobile`，点击 `+N 更多` 后为 `.more-events-pop--sheet` 且宽度跟随视口。
- 移动端全屏编辑页「删除日程」按钮增加专用布局：无描边、44px 触控高度、中性背景和红色文字；删除图标移除，按钮上方保留稳定间距。
