...
> 根目录 `AGENTS.md` 已提供日常协作所需的最小流程。本文件只保留详细版说明与边界案例。
...
- 日常快速进入仓库时，先读根目录 `AGENTS.md`
- 只有当任务涉及流程细节、例外条件、ADR 判断或门禁背景时，再展开本文件
- 如果本文件与 `AGENTS.md` 出现表述差异，以 `AGENTS.md` 指向的一级真源和当前门禁实现为准，并回收重复内容
