...
- [ ] **Step 2: 在 `MonthOptions` 类型块补开关字段**
...
- [ ] 回写 `docs/tasks/2026-06-17-month-view-drag.md` 的「实施结果」段：实际改动、与原计划偏差、验证结果、剩余问题（resize / create / narrowWeekend 精确命中 / 跨周多段幽灵条）。
- [ ] 全量门禁：`node scripts/check-docs.mjs && node scripts/check-arch.mjs && pnpm lint && pnpm -r exec tsc --noEmit && pnpm test`
